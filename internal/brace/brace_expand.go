...
// Expand runs brace-expansion expand(str, {max, maxLength}).
// Does not validate pattern length or honor nobrace (caller handles that).
func Expand(pattern string, max, maxLength int) []string {
	if max <= 0 {
		max = ExpansionMax
	}
	if maxLength <= 0 {
		maxLength = ExpansionMaxLength
	}
	return expand(pattern, max, maxLength)
}

// HasSimpleGroup is the ReDoS-safe `{…}` pre-check used by minimatch.braceExpand.
func HasSimpleGroup(pattern string) bool {
	return hasSimpleBraceGroup(pattern)
}
